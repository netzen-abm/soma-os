...
impl SovereignComplianceShield {
    ...
    pub fn enforce_regulatory_compliance_checks(
        ...
        let contains_absolute_cure_claim = message_lower.contains("cure cancer")
            || message_lower.contains("completely eliminate tumors");
        if contains_absolute_cure_claim {
            compliance_flags
                .push("VIOLATION_INDICATOR::DRUGS_AND_MAGIC_REMEDIES_ACT_1954".to_string());
        }
        ...
        let contains_prescriptive_command = message_lower.contains("prescribe")
            || message_lower.contains("take this medication to cure");
        if contains_prescriptive_command {
            compliance_flags
                .push("VIOLATION_INDICATOR::NMC_ACT_2019_PRESCRIPTION_OVERREACH".to_string());
        }
        ...
        let contains_unmasked_identifiers =
            message_lower.contains("patient name:") || message_lower.contains("phone number:");
        if contains_unmasked_identifiers {
            compliance_flags
                .push("VIOLATION_INDICATOR::DATA_PRIVACY_PHI_LEAK_DETECTION".to_string());
        }
