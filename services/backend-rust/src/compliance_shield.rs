...
impl SovereignComplianceShield {
    pub fn enforce_regulatory_compliance_checks(
        ...
        Self::check_prescriptive_language(
            &message_lower,
            &mut compliance_flags,
        );
        Self::check_identifier_exposure(
            &message_lower,
            &mut compliance_flags,
        );
        ...
        ComplianceValidationReport {
            ...
            appended_regulatory_disclaimer:
    ...
    fn check_cure_claims(message: &str, flags: &mut Vec<String>) {
        let has_cure_claim = message.contains("cure cancer")
            || message.contains("completely eliminate tumors");

        if has_cure_claim {
            flags.push(
                "VIOLATION_INDICATOR::ABSOLUTE_CURE_CLAIM"
                    .to_string(),
            );
        }
    }

    fn check_prescriptive_language(
        message: &str,
        flags: &mut Vec<String>,
    ) {
        let has_prescriptive_language = message.contains("prescribe")
            || message.contains("take this medication to cure");

        if has_prescriptive_language {
            flags.push(
                "VIOLATION_INDICATOR::PRESCRIPTIVE_OVERREACH"
                    .to_string(),
            );
        }
    }

    fn check_identifier_exposure(
        message: &str,
        flags: &mut Vec<String>,
    ) {
        let has_identifier = message.contains("patient name:")
            || message.contains("phone number:");

        if has_identifier {
            flags.push(
                "VIOLATION_INDICATOR::PERSONAL_IDENTIFIER_EXPOSURE"
                    .to_string(),
            );
        }
    }
...
mod tests {
    ...
    fn blocks_absolute_cure_claims() {
        let report =
            SovereignComplianceShield::enforce_regulatory_compliance_checks(
    ...
    fn permits_neutral_management_information() {
        let report =
            SovereignComplianceShield::enforce_regulatory_compliance_checks(
                "This is general management information.",
            );
